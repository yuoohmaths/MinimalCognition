...
This is to explore the model on cycle graphs: 
   ...
   (iii) frequency ratio between 1/10 and 10.
"""
#%%
import numpy as np
...
# Oscillation 
def oscillation(t, rate, theta=0.01):
...
# different frequency considered
the0 = 0.01
thers = [0.1, 0.5, 0.8, 0.9,
         1., 
         10/9., 10/8., 2., 3., 5., 7., 10.]
...
for ther in thers:
    the = the0*ther
    print("current frequency:", the0, the)
    if ther >= 10:
        ther_lab = str(ther)[:2]+'-'+str(ther)[3:5]
    else:
        ther_lab = str(ther)[0]+'-'+str(ther)[2:5]
    print("frequency ratio:", ther, ther_lab)
    ...
    for ph in phs:
        ...
        fp_lab = 'f'+ ther_lab + '-p' + ph_lab
        print("current comb:", ther, ph, fp_lab)
        ...
        def diff_eqs(INP,t):  
            ...
            for i in range(N):
                if i in souri:
                    Y[i] = oscillation(t, at, theta=the0) - bt*V[i]
                elif i in sinki:
                    Y[i] = oscillation(t+t_lag, at, theta=the) - bt*V[i]
                else: 
                    Y[i] = 0
